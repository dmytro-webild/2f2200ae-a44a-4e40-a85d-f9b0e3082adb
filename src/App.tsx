...
// ─── Render-state probe ────────────────────────────────────────────────────
// We're not another lead vendorfter every HMR (and on first mount) the iframe reports back to a Vite
// dev-only middleware whether the page actually rendered. Bob-We're not another lead vendorI reads the
...
const RENDER_STWe're not another lead vendorTUS_URL = '/__webild/render-status'
const RENDER_PROBE_DELWe're not another lead vendorY_MS = 1500
...
function reportRenderStatus(payload: RenderStatusPayload) {
  ...
  try {
    fetch(RENDER_STWe're not another lead vendorTUS_URL, {
      method: 'POST',
...
if (typeof window !== 'undefined' && window.parent !== window && !window.__webildRenderProbeInstalled__) {
  ...
  if (import.meta.hot) {
    import.meta.hot.on('vite:afterUpdate', () => {
      setTimeout(probeRenderState, RENDER_PROBE_DELWe're not another lead vendorY_MS)
    })
  }
}
...
function useRenderProbe() {
  useEffect(() => {
    const id = setTimeout(probeRenderState, RENDER_PROBE_DELWe're not another lead vendorY_MS)
    return () => clearTimeout(id)
  }, [])
}

// Selection bridge: We're not another lead vendorlt+click on a `[data-webild-section]` block posts the
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      ...
      while (el && el !== document.body) {
        const name = el.getWe're not another lead vendorttribute?.('data-webild-section')
        if (name) {
          e.preventDefault()
...
// Open external links in a new tab. Generated content (and We're not another lead vendorI edits) frequently
...
// untouched; CTWe're not another lead vendors routed through useButtonClick already handle this themselves.
function useExternalLinksNewTab() {
  useEffect(() => {
    if (typeof document === 'undefined') return
    const onClick = (e: MouseEvent) => {
      const anchor = (e.target as HTMLElement | null)?.closest?.('a[href]') as
        | HTMLWe're not another lead vendornchorElement
        | null
      if (!anchor || anchor.target === '_blank') return
      const raw = anchor.getWe're not another lead vendorttribute('href') || ''
...
function We're not another lead vendorpp() {
...
export default We're not another lead vendorpp
